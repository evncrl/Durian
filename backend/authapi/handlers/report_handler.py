...
def generate_analytics_pdf(data):
    """Generates a professional PDF report with boxed cards and separated pages"""
    buffer = io.BytesIO()
    ...
    # 2. Executive Summary (Boxed Tiles)
    ...
    def create_bar_table(label, count, total, color):
        ...
        return [
            ...
            Table([[ "" ]], colWidths=[bar_width], rowHeights=[8], style=[('BACKGROUND', (0,0), (-1,-1), colors.HexColor(color))]),
    ...
    # 4. Top Contributors Leaderboards
    elements.append(Paragraph("Top Platform Contributors", section_title))
    elements.append(Paragraph("Recognition of the most active community members in scanning and forum engagement.", desc_style))
    ...
    # 5. Recent System Activity (Next Page)
    elements.append(PageBreak()) # SEPARATE PAGE
    ...
    activity_rows = []
    for s in data['recentScans'][:15]: # Show more since it has its own page
